...
export default class {
  ...
  public static async Signup(client: Client, encData: string) {
    ...
    let issetIdBlock = await Model.User.IIdBlockModel.findOne({ identity: idChain.GetValidate }, (err, issetBlock) => {
      if (err) return err;
    ...
    //index 중복 확인
    do {
      let issetIdBlock2Index = await Model.User.IIdBlockModel.findOne({ index: userIndex }, (err, issetIdBlock) => {
        if (err) return err;
